...
LinearLayout.LayoutParams.MatchParent);
			sfSchedule.Appointments = getFrenchAppointments();
		...
		void spinner_ItemSelected(object sender, AdapterView.ItemSelectedEventArgs e)
		{
			...
			else if (e.Position == 1)
			{
				...
				sfSchedule.Appointments = (getSpanishAppointments());
			}
			else if (e.Position == 2)
			{
				sfSchedule.Locale = (new Locale("en-US"));
				sfSchedule.Appointments = (getEnglishAppointments());
			}
			else if (e.Position == 3)
			{
				sfSchedule.Locale = (new Locale("zh"));
				sfSchedule.Appointments = (getChineseAppointments());
			}
		}
