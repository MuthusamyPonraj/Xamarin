...
namespace SampleBrowser
{
    public class MovieTile : GridCell
    {
        ...
        private async void LoopView()         {
            ...
            LoadImage("Images/Popup_DateSelected.png", new CGRect(10 + 5, 50 + 5, 175, 75));             await Task.Delay(1800);              popUp.IsOpen = false;
            var point1 = dataGrid.RowColumnIndexToPoint(new Syncfusion.GridCommon.ScrollAxis.RowColumnIndex(2, 0));

            await Task.Delay(300);
			popUp.PopupView.AnimationMode = AnimationMode.SlideOnLeft; 			LoadImage("Images/Popup_TheatrInfo.png", new CGRect(this.dataGrid.Frame.Width - 200, point1.Y + (this.dataGrid.GetRowHeight(2) / 2) + 40, 150, 35));             await Task.Delay(1800);              popUp.IsOpen = false;             await Task.Delay(300);
			popUp.PopupView.AnimationMode = AnimationMode.Zoom;             LoadImage("Images/Popup_SelectSeats.png", new CGRect(point1.X + 10, point1.Y - 10, 175, 75));             await Task.Delay(1800); 
			popUp.IsOpen = false;
            await Task.Delay(200);             backgroundView.RemoveFromSuperview();         }          private void LoadImage(string path, CGRect rect)         {             imageView = null;             imageView = new UIImageView();             imageView.Image = UIImage.FromFile(path);
            popUp.PopupView.Frame = rect;

            imageView.Frame = new CGRect(popUp.PopupView.Frame.Left, popUp.PopupView.Frame.Top, popUp.PopupView.Frame.Width, popUp.PopupView.Frame.Height);
			popUp.PopupView.PopupStyle.BorderThickness = 0;
			popUp.PopupView.PopupStyle.BorderColor = UIColor.Clear;
			popUp.PopupView.ShowHeader = false;
			popUp.PopupView.ShowFooter = false;
			//popUp.PopupView.AppearanceMode = AppearanceMode.Custom;
			popUp.PopupView.ShowHeader = false;
			popUp.PopupView.ShowFooter = false;
			popUp.BackgroundColor = UIColor.Red; 			popUp.PopupView.ContentView = imageView;             popUp.IsOpen = true;         } 
