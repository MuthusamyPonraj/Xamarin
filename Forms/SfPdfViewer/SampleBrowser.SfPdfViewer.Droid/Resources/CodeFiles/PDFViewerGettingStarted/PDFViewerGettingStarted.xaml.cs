    ...
    public partial class PDFViewerGettingStarted : SampleView
    {
        ...
        public override void OnAppearing()
        {
            if (Device.RuntimePlatform == Device.Android)
            {
                ...
                if (!m_isPageSwitched)
                {
                    (BindingContext as GettingStartedViewModel).PdfDocumentStream = (typeof(App).GetTypeInfo().Assembly.GetManifestResourceStream("SampleBrowser.SfPdfViewer.Assets." + currentDocument + ".pdf"));
                }
            }
        }
